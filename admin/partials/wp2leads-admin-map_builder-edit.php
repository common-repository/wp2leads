...
$is_active_map_owner = MapBuilderManager::is_map_owner($decodedInfo);
$is_active_map_on_server = MapBuilderManager::is_map_on_server($decodedInfo);
$is_active_map_change_allowed = Wp2leads_License::is_dev_allowed() || $is_active_map_owner;
...
if (!$mapValid) {
    ?>
    <h3 class="title"><?php echo $activeMap->name ?></h3>
    ...
    <h2 id="wp2l-create-map-header"><?php _e('Update Map', 'wp2leads') ?></h2>
    ...
    <section id="create-map-section">
            <form id="map-generator" action="" method="post">
                ...
                <table class="form-table">
                    <tr class="alternate">
                        <td><?php _e('Current Map', 'wp2leads') ?></td>

                        <td>
                            <?php
                            if ($is_active_map_owner) {
                                ?><span class="dashicons dashicons-admin-users"></span><?php
                            }

                            if ($is_active_map_on_server) {
                                ?><span class="dashicons dashicons-admin-site"></span><?php
                            }
                            ?>
                            <strong><?php echo $activeMap->name ?></strong>
                            <?php
                            if (!empty($decodedInfo['publicMapVersion'])) {
                                ?><small> v. <?php echo $decodedInfo['publicMapVersion']; ?></small><?php
                            }
                            ?>
                            ...
                            if (!$is_active_map_owner) {
                                ?>
                                <div class="notice notice-warning inline" style="margin-bottom:0;margin-top:10px;">
                                    <p style="margin-top:5px;margin-bottom:5px">
									    <?php _e('This Map is maintained by another user. If you want fully customize it, you need to duplicate or make it your own.', 'wp2leads'); ?>

										 <a class="wp2l-map-edit button button-small" href="?page=wp2l-admin&tab=map_builder&duplicate_mapping=<?php echo $activeMap->id ?>">
                                            <?php _e('Duplicate', 'wp2leads') ?>
                                        </a>

										 <a class="wp2l-map-edit button button-small" href="?page=wp2l-admin&tab=map_builder&duplicate_mapping=<?php echo $activeMap->id ?>&change_owner=1">
                                            <?php _e('Make it my own map', 'wp2leads') ?>
                                        </a>
                                    </p>
                                </div>
                                <?php
                    ...
                    <tr>
                        ...
                        <td>
                            <?php
                            if ( $is_use_own_map_allowed && $is_active_map_change_allowed ) {
                                ?>
                    ...
                    <tr class="alternate">
                        ...
                        <td class="api-processing-holder">
                            ...
                            $selected = false;
                            $selected_starter = '';
                            $disabled_params = ( $is_use_own_map_allowed && $is_active_map_change_allowed ) ? '' : ' style="display:none" disabled';
                            ?>
                            ...
                            <?php if (!$is_use_own_map_allowed || !$is_active_map_change_allowed) echo '<strong style="display:inline-block;margin-left:15px">' .$selected_value .'</strong>'; ?>
                        ...
                        <td class="api-processing-holder">
                            <div style="width:100%;max-width:400px;">
                                <select data-current_value="<?php echo !empty( $decodedMap['keyBy'] ) ? $decodedMap['keyBy'] : '' ?>"
                                        ...
                                        id="wp2l-group-map-results-by"<?php echo ( $is_use_own_map_allowed && $is_active_map_change_allowed ) ? ' class="form-control"' : ' class="form-control disabled disabled-for-editing" disabled' ?>
                                ></select>
                    ...
                    <tr class="alternate">
                        ...
                        <td class="api-processing-holder">
                            <div id="relationship-map-holder"<?php echo ( $is_use_own_map_allowed && $is_active_map_change_allowed ) ? '' : ' class="disabled disabled-for-editing"' ?>
                                ...
                                <div class="virtual-relationship-list">
                                    <?php if(isset($decodedMap['virtual_relationships']) && count($decodedMap['virtual_relationships']) > 0): ?>
                                        <?php foreach($decodedMap['virtual_relationships'] as $vr): ?>
                                            <div class="virtual-relationship<?php echo ( $is_use_own_map_allowed && $is_active_map_change_allowed ) ? '' : ' disabled-for-editing' ?>">
                                                <div class="relationship">
                                                    <?php _e('Map Table', 'wp2leads') ?>
                                                    <select data-current_value="<?php echo $vr['table_from'] ?>" name="table_from" class="virtual-table_from" title=""<?php echo ( $is_use_own_map_allowed && $is_active_map_change_allowed ) ? '' : ' disabled' ?>>
                                                        <?php foreach($tables as $table): ?>
                                                            <option value="<?php echo $table ?>" <?php echo ($vr['table_from'] == $table ? 'selected' : ''); ?>><?php echo $table; ?></option>
                                                        <?php endforeach; ?>
                                                    </select>
                                                    . <select data-current_value="<?php echo $vr['column_from'] ?>" name="column_from" class="virtual-column_from" title=""<?php echo ( $is_use_own_map_allowed && $is_active_map_change_allowed ) ? '' : ' disabled' ?>>
                                                    ...
                                                    <?php _e('To', 'wp2leads') ?>
                                                    <select data-current_value="<?php echo $vr['table_to'] ?>" name="table_to" class="virtual-table_to" title=""<?php echo ( $is_use_own_map_allowed && $is_active_map_change_allowed ) ? '' : ' disabled' ?>>
                                                        <?php foreach($tables as $table): ?>
                                                            <option value="<?php echo $table ?>" <?php echo ($vr['table_to'] == $table ? 'selected' : ''); ?>><?php echo $table; ?></option>
                                                        <?php endforeach; ?>
                                                    </select>
                                                    . <select data-current_value="<?php echo $vr['column_to'] ?>" name="column_to" class="virtual-column_to" title=""<?php echo ( $is_use_own_map_allowed && $is_active_map_change_allowed ) ? '' : ' disabled' ?>>
                                                ...
                                                <div class="column">
                                                    ...
                                                    $selected = false;
                                                    $selected_column = '';
                                                    $disabled_params = ( $is_use_own_map_allowed && $is_active_map_change_allowed ) ? '' : ' disabled';
                                                    ?>
                    ...
                    <tr class="alternate">
                        ...
                        <td class="api-processing-holder">
                            <div id="column-comparison-holder"
                                ...
                                <?php echo ( $is_use_own_map_allowed && $is_active_map_change_allowed ) ? '' : ' class="disabled disabled-for-editing" disabled' ?>
                            ></div>
                            <?php
                            if ($is_use_own_map_allowed && $is_active_map_change_allowed) {
                                ?>
        ...
        <table class="form-table">
            ...
            <tr>
                ...
                <td>
                    <div id="titlediv">
                        <div id="titlewrap">
                            <input class="regular-text" type="text" name="mapname" id="title" value="<?php echo $activeMap->name ?>" required>
                        </div>
                    </div>
                </td>
            </tr>
            ...
            <tr>
                ...
                <td>
                    <input type="hidden" id="existing_map_id" name="map_id" value="<?php echo $activeMap->id; ?>">
