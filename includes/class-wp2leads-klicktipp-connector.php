...
$connector->logout();
*/
if(!class_exists('Wp2leads_KlicktippConnector')) {
    class Wp2leads_KlicktippConnector
    {
        const ERROR_NO_LOGIN_DATA = 'no_login_data';
        ...
        private $sessionName;
        protected $error;
        protected $connector_error;

        /**
         * Tha key of the option that keeping username value for Klick-Tip API account
         *
         * @var string
         */
        private $username_option_key = 'wp2l_klicktipp_username';

        /**
         * Tha key of the option that keeping password value for Klick-Tip API account
         *
         * @var string
         */
        private $password_option_key = 'wp2l_klicktipp_password';

        /**
         * Username for Klick-Tip API account
         *
         * @var string
         */
        private $username;
        ...
        /**
         * Password for Klick-Tip API account
         *
         * @var string
         */
        private $password;
        ...
        public function __construct($service = 'http://api.klick-tipp.com') {
            $this->username = get_option($this->username_option_key);
            $this->password = get_option($this->password_option_key);
         ...
         */
        public function get_last_error($echo = true) {
            $last_error = empty($this->error) ? '' : $this->error;
            unset($this->error); // reset

            if ($echo && !empty($last_error)) {
                $html = <<<HTML
...
HTML;
        ...
        /**
         * Get last connector error
         */
        public function get_last_connector_error() {
            $last_error = !empty($this->connector_error) ? $this->connector_error : false;

            if (!$last_error) {
                return false;
            }

            unset($this->connector_error);

            if ($last_error === self::ERROR_NO_LOGIN_DATA) {
                $last_error_array = array(
                    'error' => $last_error,
                    'message' => __('It is required to specify the correct KlickTipp credentials', 'wp2leads')
                );
            } elseif ($last_error === self::ERROR_LOGIN_FAILED) {
                $last_error_array = array(
                    'error' => $last_error,
                    'message' => $this->error
                );
        ...
        public function login()
        {
            if (empty($this->username) || empty($this->password)) {
                ...
                $this->connector_error = self::ERROR_NO_LOGIN_DATA;
                return false;
            }
            ...
            if (empty($response->error)) {
                ...
                $this->error = __('Login Failed: ', 'wp2leads') . $response->error;
                $this->connector_error = self::ERROR_LOGIN_FAILED;
                return false;
            }
        }
...
if(!class_exists('Wp2leads_KlicktippPartnerConnector')) {
    class Wp2leads_KlicktippPartnerConnector extends Wp2leads_KlicktippConnector
    {
         ...
         * @return the connector object
         */
        public function __construct($username, $developer_key, $customer_key, $service = 'https://www.klick-tipp.com/api')
        {
